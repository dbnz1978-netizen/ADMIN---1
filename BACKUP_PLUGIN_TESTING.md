...
3. PHP должен иметь следующие расширения:
   ...
   - ZIP
   - MySQL
4. Директория `/admin/backups` должна существовать и иметь права на запись
...
- ✓ В директории `/admin/backups` создается ZIP файл с именем вида `backup_YYYY-MM-DD_HH-ii-ss.zip`
...
1. Временно изменить права на директорию `/admin/backups` (chmod 555)
...
- ✓ Созданные резервные копии остаются в `/admin/backups`
...
### Безопасность 2: Авторизация

**Проверка:**
- Доступ к странице плагина без авторизации должен перенаправлять на logout.php
- Доступ к странице плагина под user (не admin) должен перенаправлять на logout.php
...
3. **Права доступа**: Требуются права на запись в директорию `/admin/backups`

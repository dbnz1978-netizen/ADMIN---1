...
- ✓ При прямом открытии URL: редирект на `/admin/logout.php`
- ✓ Отображается страница подтверждения выхода: "Выход из системы" / "Для выхода подтвердите действие."
...
- ✓ Меню "Новости" НЕ видно в сайдбаре
- ✓ При прямом открытии URL: редирект на `/admin/logout.php`
- ⚠️ Страница настроек `/plugins/news-plugin/pages/settings/access.php` требует роль admin
- ⚠️ При попытке доступа не-администратора происходит редирект на logout
...
**Ожидаемый результат:**
- ✓ Редирект на `/admin/logout.php`
- ✓ Отображается страница подтверждения выхода
...
**Ожидаемый результат:**
- ✓ В логах есть запись: "Отказ в доступе к плагину 'news-plugin': пользователь ID=... (роль=user)"
...
- [ ] Редирект на /admin/logout.php происходит при отказе в доступе
...
- "Настройки доступа к плагину 'news-plugin' обновлены"
- "Отказ в доступе к плагину 'news-plugin'"
...
**Проблема:** Редирект на /admin/logout.php не происходит
**Решение:** Проверьте, что на странице вызывается pluginAccessGuard() и headers еще не были отправлены

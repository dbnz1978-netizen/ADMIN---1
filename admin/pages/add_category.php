<?php
/**
 * Файл: /admin/pages/add_category.php
 *
 * Назначение:
 * - Добавление и редактирование записей (например: записи/категорий) в админ-панели.
 * - Работает с одной таблицей (задаётся переменной $categoryTable).
...
$categoryTable = 'pages';                 // например: 'category2', 'category', 'pages'

// related_table — "тип"/контекст текущих редактируемых записей
$relatedTable = 'category';              // например: 'product'

// related_table — фильтр ТОЛЬКО для выбора родительской записи (parent_search и проверка parent_id)
$parentRelatedTable = 'category';           // например: 'pages' (или 'category', 'category', и т.д.)
...
if ($isEditMode && $itemId) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM {$categoryTable} WHERE id = ? AND related_table = ? AND users_id = ? LIMIT 1");
        ...
        if ($item) {
            ...
            if ($defaultParentId > 0) {
                $stmtP = $pdo->prepare("SELECT naime FROM {$categoryTable} WHERE id = ? AND related_table = ? AND users_id = ? LIMIT 1");
        ...
        logEvent("Ошибка загрузки раздела ID=$itemId ({$categoryTable}): " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
    }
}
...
if (isset($_GET['action']) && $_GET['action'] === 'parent_search') {
    ...
    try {
        ...
        logEvent("Ошибка parent_search ({$categoryTable}): " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
    ...
    if (!validateCsrfToken($csrfToken)) {
        ...
        if ($parent_id > 0) {
            if ($isEditMode && $itemId && $parent_id === $itemId) {
                ...
                $stmt = $pdo->prepare("SELECT id FROM {$categoryTable} WHERE id = ? AND related_table = ? AND users_id = ? LIMIT 1");
        ...
        // ---------------------------------------------------------------
        if (empty($errors)) {
            $sql = "SELECT id FROM {$categoryTable} WHERE url = ? AND related_table = ? AND users_id = ?";
        ...
        if (empty($errors)) {
            try {
                ...
                if ($isEditMode && $itemId) {
                    ...
                    logEvent("Обновлён раздел {$categoryTable} ID=$itemId related_table={$relatedTable} users_id=$currentUserId", LOG_INFO_ENABLED, 'info');
                    // Закрываем соединение при завершении скрипта
                    register_shutdown_function(function() {
                        if (isset($pdo)) {
                            $pdo = null; 
                    ...
                    logEvent("Создан новый раздел {$categoryTable} ID=$newId related_table={$relatedTable} users_id=$currentUserId", LOG_INFO_ENABLED, 'info');
                    // Закрываем соединение при завершении скрипта
                    register_shutdown_function(function() {
                        if (isset($pdo)) {
                            $pdo = null; 
                ...
                logEvent("Ошибка сохранения раздела {$categoryTable}: " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
            }
        }
    }
}
    ...
    <main class="main-content">
        ...
        <form method="post">
            <div class="form-section">
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        <label class="form-label">Название каталога <span class="text-danger">*</span></label>
            ...
            <div class="mb-5">
                ...
                <div class="form-text">Отображается на странице каталога.</div>
            ...
            <div class="row mb-3">
                <div class="col-12">
                    ...
                    <div class="form-text"><?= escape('Активная категория') ?></div>
                </div>
            </div>

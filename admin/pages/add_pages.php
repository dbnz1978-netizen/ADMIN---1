<?php
/**
 * Файл: /admin/pages/add_pages.php
 *
 * Назначение:
 * - Добавление и редактирование страницы (например: страницы/категорий) в админ-панели.
 * - Работает с одной таблицей (задаётся переменной $catalogTable).
 * - Родительская связь хранится в колонке author (ID родителя).
 * - Тип/контекст страницы хранится в related_table (например: 'pages').
...
// =============================================================================
// Загрузка страницы для редактирования (+ фильтр users_id)
// =============================================================================
if ($isEditMode && $itemId) {
    try {
        // Читаем редактируемую страницы (текущий related_table + users_id)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCsrfToken($csrfToken)) {
        ...
        logEvent("CSRF ошибка в pages_list.php", LOG_ERROR_ENABLED, 'error');
    } else {
        ...
        if (empty($errors)) {
            try {
                ...
                if ($isEditMode && $itemId) {
                    ...
                    header("Location: add_pages.php?id=$itemId");                    
                    exit;
                    ...
                    header("Location: add_pages.php?id=" . $newId);
                    exit;
                }
...
$titlemeta = $isEditMode ? 'Редактирование страницы' : 'Добавление страницы';
        ...
        <form method="post">
            ...
            <div class="row mb-3">
                <div class="col-12">
                    ...
                    <div class="form-text">Активная.</div>
                </div>
            </div>

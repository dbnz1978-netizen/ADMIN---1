<?php
/**
 * Файл: /admin/pages/add_pages_extra.php
 *
 * Назначение:
 * - Добавление и редактирование страниы в таблице с дополнительной информацией (pages_extra).
 ...
 * - users_id автоматически заполняется из $_SESSION['user_id'] при создании страниы.
...
// =============================================================================
$catalogTable = 'pages_extra'; // таблица pages_extra

// Имя таблицы для проверки author (меняется в настройках)
$authorCheckTable = 'pages';
...
try {
    ...
    logEvent("Ошибка инициализации add_pages_extra.php: " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
    // Закрываем соединение при завершении скрипта
    register_shutdown_function(function() {
        if (isset($pdo)) {
            $pdo = null; 
...
if ($authorFromGet > 0) {
    try {
        // Проверяем существование страниы в таблице $authorCheckTable
        ...
        // Если страниы не найдена - редирект на product_list.php
        if (!$authorExists) {
...
// =============================================================================
// Загрузка страниы для редактирования (+ фильтр users_id)
// =============================================================================
if ($isEditMode && $itemId) {
    try {
        // Читаем страниы с проверкой users_id
        ...
        if ($item) {
            ...
            $errors[] = 'Страница не найдена (или не принадлежит вашему пользователю).';
            // Закрываем соединение при завершении скрипта
            register_shutdown_function(function() {
                if (isset($pdo)) {
                    $pdo = null; 
        ...
        logEvent("Ошибка загрузки страниы ID=$itemId ({$catalogTable}): " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCsrfToken($csrfToken)) {
        ...
        logEvent("CSRF ошибка в add_pages_extra.php", LOG_ERROR_ENABLED, 'error');
    } else {
        ...
        if (empty($errors) && $authorFromGet > 0) {
            try {
                // Проверяем существование страниы в таблице $authorCheckTable ПЕРЕД сохранением
        ...
        if (empty($errors)) {
            try {
                if ($isEditMode && $itemId) {
                    ...
                    $successMessages[] = 'Страница успешно обновлена';
                    logEvent("Обновлена страница {$catalogTable} ID=$itemId author={$authorFromGet} users_id={$currentUserId}", LOG_INFO_ENABLED, 'info');
                    // Закрываем соединение при завершении скрипта
                    register_shutdown_function(function() {
                        if (isset($pdo)) {
                            $pdo = null; 
                    ...
                    header("Location: add_pages_extra.php?author=$authorFromGet&id=$itemId");
                    exit;
                    ...
                    $successMessages[] = 'Страница успешно создана';
                    logEvent("Создана страница {$catalogTable} ID=$newId author={$authorFromGet} users_id={$currentUserId}", LOG_INFO_ENABLED, 'info');
                    // Закрываем соединение при завершении скрипта
                    register_shutdown_function(function() {
                        if (isset($pdo)) {
                            $pdo = null; 
                    ...
                    header("Location: add_pages_extra.php?author=$authorFromGet&id=$newId");
                    exit;
                }
...
$titlemeta = $isEditMode ? 'Управление страцами' : 'Добавление страниы';
        ...
        <form method="post">
            <div class="form-section">
                ...
                <div class="row align-items-center mb-5">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="style" class="form-select">
                            <option value="css_1" <?= ($formStyle === 'css_1') ? 'selected' : '' ?>><?= escape('Полная ширина') ?></option>
                            <option value="css_2" <?= ($formStyle === 'css_2') ? 'selected' : '' ?>><?= escape('Ограниченная ширина') ?></option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-4 mt-4">
                    ...
                    <div class="form-text">Сохраняется в JSON поле data.description</div>
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        ...
                        <div class="form-text"><?= escape('Активная страница') ?></div>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check-lg"></i> <?= escape($isEditMode ? 'Сохранить' : 'Создать') ?>
                    </button>
                </div>

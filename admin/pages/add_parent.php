<?php
/**
 * Файл: /admin/pages/add_parent.php
 *
 * Назначение:
 * - Добавление и редактирование записей (например: записи/категорий) в админ-панели.
 * - Работает с одной таблицей (задаётся переменной $parentTable).
 ...
 * - parent_description          -> HTML описание (из редактора)
...
if ($isEditMode && $itemId) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM {$parentTable} WHERE id = ? AND related_table = ? AND users_id = ? LIMIT 1");
        ...
        if ($item) {
            ...
            $text = sanitizeHtmlFromEditor((string)($dataArr['parent_description'] ?? ''));
            ...
            if ($defaultParentId > 0) {
                $stmtP = $pdo->prepare("SELECT naime FROM {$parentTable} WHERE id = ? AND related_table = ? AND users_id = ? LIMIT 1");
            ...
            header("Location: parent_list.php");
            exit;
        }
    } catch (PDOException $e) {
        $errors[] = 'Ошибка загрузки данных';
        logEvent("Ошибка загрузки раздела ID=$itemId ({$parentTable}): " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
    }
}
...
if (isset($_GET['action']) && $_GET['action'] === 'parent_search') {
    ...
    try {
        ...
        logEvent("Ошибка parent_search ({$parentTable}): " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
    ...
    if (!validateCsrfToken($csrfToken)) {
        ...
        if ($parent_id > 0) {
            if ($isEditMode && $itemId && $parent_id === $itemId) {
                ...
                $stmt = $pdo->prepare("SELECT id FROM {$parentTable} WHERE id = ? AND related_table = ? AND users_id = ? LIMIT 1");
        ...
        // ---------------------------------------------------------------
        if (empty($errors)) {
            $sql = "SELECT id FROM {$parentTable} WHERE url = ? AND related_table = ? AND users_id = ?";
        ...
        if (empty($errors)) {
            try {
                ...
                if ($isEditMode && $itemId) {
                    ...
                    logEvent("Обновлён раздел {$parentTable} ID=$itemId related_table={$relatedTable} users_id=$currentUserId", LOG_INFO_ENABLED, 'info');
                    // Закрываем соединение при завершении скрипта
                    register_shutdown_function(function() {
                        if (isset($pdo)) {
                            $pdo = null; 
                    ...
                    header("Location: add_parent.php?id=$itemId");
                    exit;
                    ...
                    logEvent("Создан новый раздел {$parentTable} ID=$newId related_table={$relatedTable} users_id=$currentUserId", LOG_INFO_ENABLED, 'info');
                    // Закрываем соединение при завершении скрипта
                    register_shutdown_function(function() {
                        if (isset($pdo)) {
                            $pdo = null; 
                    ...
                    header("Location: add_parent.php?id=" . $newId);
                    exit;
                }

            } catch (PDOException $e) {
                $errors[] = 'Ошибка сохранения данных';
                logEvent("Ошибка сохранения раздела {$parentTable}: " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
            }
        }
    }
}
...
$titlemeta = $isEditMode ? 'Редактирование каталога' : 'Добавление каталога';
    ...
    <main class="main-content">
        ...
        <form method="post">
            <div class="form-section">
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        <label class="form-label">Родительский каталог</label>
                ...
                <div id="image-management-section-<?php echo $sectionId; ?>">
                    ...
                    <div class="form-text">Не более 5 шт</div>
                </div>
            ...
            <div class="row mb-3">
                <div class="col-12">
                    ...
                    <div class="form-text"><?= escape('Активная') ?></div>
                </div>
            </div>

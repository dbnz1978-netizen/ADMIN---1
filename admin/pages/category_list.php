<?php
/**
 * Файл: /admin/pages/category_list.php
 *
 * Назначение:
 ...
 *
 * Важно:
 * - Таблица каталога задаётся в настройках ($categoryTable) — меняется в одном месте.
 * - Отбор записей идёт по related_table (например 'pages') — это удобно, если в одной таблице
 ...
 *   Здесь $categoryTable задаётся вручную (глобальная настройка).
 */
...
// 1) Имя таблицы каталога: можно быстро переключать (category / category2 / и т.д.)
$categoryTable = 'pages';
...
try {
    ...
    // ★★★ ПРОВЕРКА ПРАВ ДОСТУПА К СТРАНИЦИ ★★★
...
$search_parent = trim($_GET['search_parent'] ?? '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['user_ids'])) {
    ...
    if (!empty($userIds)) {
        try {
                ...
                case 'delete':
                    if ($isTrash) {
                        ...
                        foreach ($userIds as $userId) {
                            $stmt = $pdo->prepare("DELETE FROM {$categoryTable} WHERE id = ? AND status = 0 AND related_table = ? AND users_id = ?");
                            $stmt->execute([$userId, $RELATED_TABLE_FILTER, $currentUserId]);
                        }
                        $successMessages[] = 'Записи каталога успешно удалены';
                    } else {
                        // Перемещение в корзину
                        $stmt = $pdo->prepare("UPDATE {$categoryTable} SET status = 0 WHERE related_table = ? AND id IN ($placeholders) AND users_id = ?");
                        $stmt->execute(array_merge([$RELATED_TABLE_FILTER], $userIds, [$currentUserId]));
                        $successMessages[] = 'Записи каталога перемещены в корзину';
                    }
                    break;

                case 'restore':
                    // Восстановление из корзины
                    $stmt = $pdo->prepare("UPDATE {$categoryTable} SET status = 1 WHERE related_table = ? AND id IN ($placeholders) AND users_id = ?");
                    $stmt->execute(array_merge([$RELATED_TABLE_FILTER], $userIds, [$currentUserId]));
                    $successMessages[] = 'Записи каталога восстановлены';
                    break;
            ...
            logEvent("Выполнено массовое действие '$action' администратором ID: $adminId над {$categoryTable} IDs: " . implode(',', $userIds) . " — related_table={$RELATED_TABLE_FILTER} — users_id=$currentUserId — IP: $ip", LOG_INFO_ENABLED, 'info');

        } catch (PDOException $e) {
            $errors[] = 'Ошибка при выполнении операции';
            logEvent("Ошибка БД при массовом действии '$action' ({$categoryTable}): " . $e->getMessage() . " — ID админа: " . ($user['id'] ?? 'unknown') . " — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_ERROR_ENABLED, 'error');
        }
    } else {
...
try {
    ...
    if ($search_parent !== '') {
        ...
        $query .= " AND EXISTS (
                        SELECT 1
                        FROM {$categoryTable} p
                        WHERE p.id = {$categoryTable}.author
                           AND p.status = 1
                           AND p.related_table = :parent_related_table_search
                           AND p.users_id = :users_id_search
                           AND p.naime LIKE :search_parent_search
                    )";
    ...
    // -------------------------------------------------------------------------
    // Подгружаем имена родителей (author = ID родителя) ТОЛЬКО для текущего users_id
    // -------------------------------------------------------------------------
    $parentMap = [];
    $parentIds = [];
    ...
    if (!empty($parentIds)) {
        ...
        $pStmt = $pdo->prepare("SELECT id, naime FROM {$categoryTable} WHERE related_table = ? AND users_id = ? AND id IN ($ph) AND status = 1");
        $pStmt->execute(array_merge([$parentRelatedTable, $currentUserId], $parentIds));

        while ($row = $pStmt->fetch(PDO::FETCH_ASSOC)) {
            $parentMap[(int)$row['id']] = $row['naime'];
        }
    }

    foreach ($users as &$u) {
        $pid = (int)($u['author'] ?? 0);
        $u['parent_name'] = $pid > 0 ? ($parentMap[$pid] ?? '') : '';
    }
    unset($u);
    ...
    logEvent("Ошибка базы данных при загрузке списка {$categoryTable}: " . $e->getMessage() . " — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_ERROR_ENABLED, 'error');
}
...
$titlemeta = 'Управление каталогом';
    ...
    <main class="main-content">
        ...
        <div class="content-card table-card">
            <h3 class="card-title">
                ...
                <?= $isTrash ? escape('Корзина каталога') : escape('Каталог') ?>
            </h3>
            ...
            <?php if (!$isTrash): ?>
                <?php
                    $trashCountStmt = $pdo->prepare("SELECT COUNT(*) FROM {$categoryTable} WHERE related_table = ? AND status = 0 AND users_id = ?");
                    $trashCountStmt->execute([$RELATED_TABLE_FILTER, $currentUserId]);
                    $trashCount = (int)$trashCountStmt->fetchColumn();
                ?>
                <?php if ($trashCount > 0): ?>
                    <a href="?folder=category&amp;file=category_list&amp;trash=1" class="btn btn-outline-danger mb-3">
                        <i class="bi bi-trash"></i> Корзина
                        <span class="badge bg-danger"><?= escape((string)$trashCount) ?></span>
                    </a>
                <?php endif; ?>
            <?php else: ?>
                <a href="?folder=category&amp;file=category_list" class="btn btn-outline-primary mb-3">
                    <i class="bi bi-arrow-left"></i> Назад к каталогу
                </a>
            <?php endif; ?>

            <a href="add_category.php" class="btn btn-primary ms-2 mb-3">
                <i class="bi bi-plus-circle"></i> Добавить каталог
            </a>
                ...
                <div class="row align-items-center">
                    <div class="col-md-7">
                        <form method="GET" class="d-flex mb-3">
                            <input type="hidden" name="folder" value="category">
                            <input type="hidden" name="file" value="category_list">
                            <?php if ($isTrash): ?>
                            ...
                            <div class="input-group" style="gap: 2px;">
                                ...
                                <?php if ($search !== '' || $search_parent !== ''): ?>
                                    <a href="?folder=category&amp;file=category_list<?= $isTrash ? '&amp;trash=1' : '' ?>"
                ...
                <table class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th><?= escape('Название') ?></th>
                            <th><?= escape('Родительская категория') ?></th>
                            <th width="80" class="text-center"><?= escape('URL') ?></th>
                            <th><?= escape('Дата') ?></th>
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-4">
                                    ...
                                    <p class="mt-2 auth-subtitle">
                                        <?= $isTrash ? escape('Корзина пуста') : escape('Записи не найдены') ?>
                            ...
                            <?php foreach ($users as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span class="badge bg-success"><?= escape('ID: ') . (int)$row['id'] ?></span>
                                    </td>
                                        ...
                                        <?php if (!empty($row['parent_name'])): ?>
                                            <?php
                                                $parentLink = '?' . http_build_query(array_filter([
                                                    'folder' => $_GET['folder'] ?? 'category',
                                                    'file'   => $_GET['file'] ?? 'category_list',
                                            ...
                                            ?>
                                            <a href="<?= escape($parentLink) ?>" class="text-decoration-none">
                                                <?= escape($row['parent_name']) ?>
                                            </a>
                                        <?php else: ?>

<?php
/**
 * Файл: /admin/pages/pages_extra_list.php
 *
 * Назначение:
 * - Админ-страница со списком дополнительных элементов каталога (таблица pages_extra).
 * - Поддерживает поиск по названию.
 * - Поддерживает "корзину" (status=0) и активные страниы (status=1).
 ...
 *
 * Важно:
 * - Таблица pages_extra задаётся в настройках ($catalogTable) — меняется в одном месте.
 * - НЕТ фильтра related_table — все страниы из этой таблицы.
...
// Имя таблицы pages_extra
$catalogTable = 'pages_extra';

// Имя таблицы для проверки author (Привязка страниы к родительской таблице)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['user_ids'])) {
    ...
    if (!empty($userIds)) {
            ...
            switch ($action) {
                case 'delete':
                    if ($isTrash) {
                        ...
                        $successMessages[] = 'Страниы успешно удалены';
                    } else {
                        ...
                        $successMessages[] = 'Страниы перемещены в корзину';
                    }
                    break;
                ...
                case 'restore':
                    ...
                    $stmt->execute(array_merge($userIds, [$currentUserId, $authorId]));
                    $successMessages[] = 'Страниы восстановлены';
                    break;
        ...
        $errors[] = 'Не выбраны страниы для действия.';
    }
}

// =============================================================================
// ПОЛУЧЕНИЕ ДАННЫХ ИЗ ТАБЛИЦЫ pages_extra (с фильтром users_id И author)
    ...
    <main class="main-content">
        ...
        <div class="content-card table-card">
            <h3 class="card-title">
                ...
                <?= $isTrash ? escape('Корзина описание страниы') : 'Описание страниы' ?>
            </h3>
            ...
            <a href="add_pages_extra.php?author=<?= $authorId ?>" class="btn btn-primary ms-2 mb-3">
                <i class="bi bi-plus-circle"></i> Добавить описание страниы
            </a>
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-4">
                                    ...
                                    <p class="mt-2 auth-subtitle">
                                        <?= $isTrash ? escape('Корзина пуста') : escape('Страниы не найдены') ?>
                        ...
                        <?php else: ?>
                            <?php foreach ($users as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="add_pages_extra.php?id=<?= (int)$row['id'] ?>&author=<?= $authorId ?>" class="user-link text-decoration-none">
                                            <?= escape($row['naime'] ?? '') ?>
                                        </a>
                                    ...
                                    <td>
                                        <div class="btn-group" style="gap: 4px;">
                                            <a href="add_pages_extra.php?id=<?= (int)$row['id'] ?>&author=<?= $authorId ?>"

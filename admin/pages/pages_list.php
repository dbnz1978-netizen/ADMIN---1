<?php
/**
 * Файл: /admin/pages/pages_list.php
 *
 * Назначение:
 ...
 * - Отбор страницы идёт по related_table (например 'pages') — это удобно, если в одной таблице
...
try {
    ...
    foreach ($users as &$u) {
        $pid = (int)($u['author'] ?? 0);
        if ($pid > 0 && isset($parentMap[$pid])) {
            $u['parent_name'] = $parentMap[$pid]['name'];
            $u['parent_url'] = $parentMap[$pid]['url'];
            // Формируем полную структуру URL: категория/товар
            $u['full_url'] = !empty($parentMap[$pid]['url']) && !empty($u['url']) 
                ? trim($parentMap[$pid]['url'], '/') . '/' . trim($u['url'], '/') 
                : $u['url'];
        } else {
            $u['parent_name'] = '';
            $u['parent_url'] = '';
                    ...
                    <?php else: ?>
                        <?php foreach ($users as $row): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="add_pages.php?id=<?= (int)$row['id'] ?>" class="user-link text-decoration-none">
                                        <?= escape($row['naime'] ?? '') ?>
                                    </a>
                                ...
                                <td>
                                    <div class="btn-group" style="gap: 4px;">
                                        <a href="add_pages.php?id=<?= (int)$row['id'] ?>"

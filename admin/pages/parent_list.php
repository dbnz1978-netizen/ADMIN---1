<?php
/**
 * Файл: /admin/pages/parent_list.php
 *
 * Назначение:
 ...
 *
 * Важно:
 * - Таблица каталога задаётся в настройках ($parentTable) — меняется в одном месте.
 ...
 *   Здесь $parentTable задаётся вручную (глобальная настройка).
 */
...
// 1) Имя таблицы каталога: можно быстро переключать (parent / parent2 / и т.д.)
$parentTable = 'pages';

// 2) Фильтр related_table (какой "контекст" показываем в этом списке)
$RELATED_TABLE_FILTER = 'parent';
...
/**
 * Рекурсивно получает полный путь URL категории (категория1/категория2/текущая)
 *
 * @param PDO $pdo
 * @param string $table
 ...
 * @param int $maxDepth Максимальная глубина рекурсии (защита от циклов)
 * @return string Полный URL путь
 */
function getFullCategoryUrl($pdo, $table, $categoryId, $usersId, $relatedTable, $maxDepth = 10) {
    static $cache = [];
    static $depth = 0;
    ...
    $cacheKey = "{$table}_{$categoryId}_{$usersId}";
    ...
    try {
        $stmt = $pdo->prepare("SELECT id, url, author FROM {$table} WHERE id = ? AND related_table = ? AND users_id = ? AND status = 1 LIMIT 1");
        $stmt->execute([$categoryId, $relatedTable, $usersId]);
        $category = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$category || empty($category['url'])) {
            $cache[$cacheKey] = '';
            $depth--;
            return '';
        }
        
        $parentId = (int)$category['author'];
        $parentUrl = '';
        
        if ($parentId > 0) {
            $parentUrl = getFullCategoryUrl($pdo, $table, $parentId, $usersId, $relatedTable, $maxDepth);
        }
        
        $fullUrl = trim($parentUrl . '/' . $category['url'], '/');
        $cache[$cacheKey] = $fullUrl;
        $depth--;
        return $fullUrl;
            ...
            switch ($action) {
                case 'delete':
                    if ($isTrash) {
                        ...
                        $successMessages[] = 'Каталога успешно удалены';
                    } else {
                        ...
                        $successMessages[] = 'Каталога перемещены в корзину';
                    }
                    break;
                ...
                case 'restore':
                    ...
                    $stmt->execute(array_merge([$RELATED_TABLE_FILTER], $userIds, [$currentUserId]));
                    $successMessages[] = 'Каталоги восстановлены';
                    break;
                ...
                case 'trash':
                    ...
                    $stmt = $pdo->prepare("UPDATE {$parentTable} SET status = 0 WHERE related_table = ? AND id IN ($placeholders) AND users_id = ?");
                    $stmt->execute(array_merge([$RELATED_TABLE_FILTER], $userIds, [$currentUserId]));
                    $successMessages[] = 'Каталоги перемещены в корзину';
                    break;
...
try {
    ...
    if (!empty($parentIds)) {
        ...
        $pStmt = $pdo->prepare("SELECT id, naime FROM {$parentTable} WHERE related_table = ? AND users_id = ? AND id IN ($ph) AND status = 1");
    ...
    foreach ($users as &$u) {
        ...
        $u['full_url'] = getFullCategoryUrl($pdo, $parentTable, (int)$u['id'], $currentUserId, $parentRelatedTable);
    }
    unset($u);
    ...
    <main class="main-content">
        ...
        <div class="content-card table-card">
            <h3 class="card-title">
                ...
                <?= $isTrash ? escape('Корзина') : escape('Каталог') ?>
            </h3>
            ...
            <?php if (!$isTrash): ?>
                ...
                ?>
                <?php if ($trashCount > 0): ?>
                    <a href="?folder=parent&amp;file=parent_list&amp;trash=1" class="btn btn-outline-danger mb-3">
                        <i class="bi bi-trash"></i> Корзина
                        <span class="badge bg-danger"><?= escape((string)$trashCount) ?></span>
                    </a>
                <?php endif; ?>
            <?php else: ?>
                <a href="?folder=parent&amp;file=parent_list" class="btn btn-outline-primary mb-3">
                    <i class="bi bi-arrow-left"></i> Назад к каталогу
                </a>
            <?php endif; ?>

            <a href="add_parent.php" class="btn btn-primary ms-2 mb-3">
                        ...
                        <form method="GET" class="d-flex mb-3">
                            ...
                            <div class="input-group" style="gap: 2px;">
                                ...
                                <?php if ($search !== '' || $search_parent !== ''): ?>
                                    <a href="?folder=parent&amp;file=parent_list<?= $isTrash ? '&amp;trash=1' : '' ?>"
                        ...
                        <?php else: ?>
                            <?php foreach ($users as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="add_parent.php?id=<?= (int)$row['id'] ?>" class="user-link text-decoration-none">
                                            <?= escape($row['naime'] ?? '') ?>
                                        </a>
                                    ...
                                    <td class="text-center">
                                        <?php if (!empty($row['full_url'])): ?>
                                            <a href="/<?= escape($row['full_url']) ?>"
                                               ...
                                               title="Открыть в новом окне: /<?= escape($row['full_url']) ?>">
                                    ...
                                    <td>
                                        <div class="btn-group" style="gap: 4px;">
                                            <a href="add_parent.php?id=<?= (int)$row['id'] ?>"

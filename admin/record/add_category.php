<?php
/**
 * Файл: /admin/record/add_category.php
 *
 * Назначение:
 ...
 * - Тип/контекст записи хранится в related_table (например: 'record').
...
// related_table — фильтр ТОЛЬКО для выбора родительской записи (catalog_search и проверка catalog_id)
$parentRelatedTable = 'category';           // например: 'record' (или 'category', 'category', и т.д.)
...
try {
    ...
    logEvent("Ошибка инициализации record_list.php: " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
    // Закрываем соединение при завершении скрипта
    register_shutdown_function(function() {
        if (isset($pdo)) {
            $pdo = null; 
...
// =============================================================================
// Функция рекурсивного построения пути категорий
// =============================================================================
function buildCategoryPath($pdo, $itemId, $catalogTable, $relatedTable, $currentUserId, $maxDepth = 10, &$visited = []) {
    if ($maxDepth <= 0 || $itemId <= 0 || isset($visited[$itemId])) {
        return '';
    }
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT author, url 
            FROM {$catalogTable} 
            WHERE id = ? AND related_table = ? AND users_id = ? 
            LIMIT 1
        ");
        ...
        $parentId = (int)$currentItem['author'];
        $currentUrl = trim($currentItem['url']);
        
        if ($parentId <= 0) {
            // Нет родителя - возвращаем только текущий URL
            return $currentUrl;
        }
        
        // Рекурсивно строим путь родителя
        $parentPath = buildCategoryPath($pdo, $parentId, $catalogTable, $relatedTable, $currentUserId, $maxDepth - 1, $visited);
        ...
        logEvent("Ошибка buildCategoryPath для ID=$itemId: " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
        return '';
    }
}
...
if ($isEditMode && $itemId) {
    try {
        ...
        if ($item) {
            ...
            $categoryFullPath = buildCategoryPath($pdo, $itemId, $catalogTable, $relatedTable, $currentUserId);
            ...
            if ($defaultParentId > 0) {
                $stmtP = $pdo->prepare("SELECT naime FROM {$catalogTable} WHERE id = ? AND related_table = ? AND users_id = ? AND status = 1 LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCsrfToken($csrfToken)) {
        ...
        logEvent("CSRF ошибка в record_list.php", LOG_ERROR_ENABLED, 'error');
    } else {
        ...
        if ($parent_id > 0) {
            if ($isEditMode && $itemId && $parent_id === $itemId) {
                ...
                $stmt = $pdo->prepare("SELECT id FROM {$catalogTable} WHERE id = ? AND related_table = ? AND users_id = ? AND status = 1 LIMIT 1");
                ...
                <div class="row align-items-center mb-4">
                    <div class="col-lg-6">
                        <h3 class="card-title d-flex align-items-center gap-2 mb-0">
                            ...
                            <?= $isEditMode ? 'Редактирование каталога' : 'Добавление каталога' ?>
                        </h3>
                    </div>
                    <div class="col-lg-6 text-end">
                        <?php if ($isEditMode && $itemId && $categoryFullPath): ?>
                            <a href="/<?= escape($parentRelatedTable) ?>/<?= escape($categoryFullPath) ?>" target="_blank" 

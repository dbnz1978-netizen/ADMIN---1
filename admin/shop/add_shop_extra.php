<?php
/**
 * Файл: /admin/shop/add_shop_extra.php
 *
 * Назначение:
 * - Добавление и редактирование записей в таблице с дополнительной информацией (shop_extra).
...
// =============================================================================
$catalogTable = 'shop_extra'; // таблица shop_extra
...
try {
    ...
    logEvent("Ошибка инициализации add_shop_extra.php: " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
    // Закрываем соединение при завершении скрипта
    register_shutdown_function(function() {
        if (isset($pdo)) {
            $pdo = null; 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCsrfToken($csrfToken)) {
        ...
        logEvent("CSRF ошибка в add_shop_extra.php", LOG_ERROR_ENABLED, 'error');
    } else {
        ...
        if (empty($errors)) {
            try {
                if ($isEditMode && $itemId) {
                    ...
                    header("Location: add_shop_extra.php?author=$authorFromGet&id=$itemId");
                    exit;
                    ...
                    header("Location: add_shop_extra.php?author=$authorFromGet&id=$newId");
                    exit;
                }
        ...
        <form method="post">
            <div class="form-section">
                <h3 class="card-title">
                    ...
                    <?= $isEditMode ? 'Редактировать описание товара' : 'Добавить описание товара' ?>
                </h3>

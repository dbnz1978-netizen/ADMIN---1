<?php
/**
 * Файл: /admin/shop/catalog_list.php catalog
 *
 * Назначение:
 ...
 * - Отбор записей идёт по related_table (например 'shop') — это удобно, если в одной таблице
...
function getFullCategoryUrl($pdo, $table, $categoryId, $usersId, $relatedTable, $maxDepth = 10) {
    ...
    try {
        ...
        $catalogId = (int)$category['author'];
        $catalogUrl = '';
        
        if ($catalogId > 0) {
            $catalogUrl = getFullCategoryUrl($pdo, $table, $catalogId, $usersId, $relatedTable, $maxDepth);
        }
        
        $fullUrl = trim($catalogUrl . '/' . $category['url'], '/');
...
try {
    ...
    if (!empty($catalogIds)) {
        ...
        $pStmt = $pdo->prepare("SELECT id, naime FROM {$catalogTable} WHERE related_table = ? AND users_id = ? AND id IN ($ph) AND status = 1");
        $pStmt->execute(array_merge([$catalogRelatedTable, $currentUserId], $catalogIds));

        while ($row = $pStmt->fetch(PDO::FETCH_ASSOC)) {
            $catalogMap[(int)$row['id']] = $row['naime'];
        }
    }
    ...
    foreach ($users as &$u) {
        ...
        $u['full_url'] = getFullCategoryUrl($pdo, $catalogTable, (int)$u['id'], $currentUserId, $catalogRelatedTable);
    }
    unset($u);
    ...
    <main class="main-content">
        ...
        <div class="content-card table-card">
            <div class="row align-items-center mb-4">
                <div class="col-lg-6">
                    <h3 class="card-title d-flex align-items-center gap-2 mb-0">
                        ...
                        <?= $isTrash ? 'Корзина' : 'Управление каталогом' ?>
                    </h3>
                </div>
                <div class="col-lg-6 text-end">
                    <a href="add_catalog.php"
            ...
            <?php if (!$isTrash): ?>
                ...
                <?php endif; ?>
            <?php else: ?>
                <a href="catalog_list.php" class="btn btn-outline-primary mb-3">
                        ...
                        <?php else: ?>
                            <?php foreach ($users as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="add_catalog.php?id=<?= (int)$row['id'] ?>" class="user-link text-decoration-none">
                                            <?= escape($row['naime'] ?? '') ?>
                                        </a>
                                    ...
                                    <td>
                                        <div class="btn-group" style="gap: 4px;">
                                            <a href="add_catalog.php?id=<?= (int)$row['id'] ?>"

 ...
 * Назначение:          Добавление и редактирование записей в таблице с дополнительным контентом (news_extra_content).
 *                      ФИЛЬТРАЦИЯ ПО users_id = $_SESSION['user_id'] при загрузке и users_id автоматически заполняется при создании.
 * Важно:               - news_id (родительский ID) берётся из GET параметра: ?news_id=1

 ...
 * Назначение:          Скрипт удаления плагина News Plugin
...
function plugin_uninstall($pdo)
{
    try {
        ...
        $pdo->exec("DROP TABLE IF EXISTS `news_extra_content`");
        $pdo->exec("DROP TABLE IF EXISTS `news_articles`");
        $pdo->exec("DROP TABLE IF EXISTS `news_categories`");
        
        return [
            'success' => true,
            'message' => 'Плагин "Система управления новостями" успешно удален. Таблицы удалены.'
        ];

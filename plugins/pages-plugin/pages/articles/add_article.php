 ...
 * Назначение:          Добавление и редактирование страниц в админ-панели.
 *                      Работает с таблицей pages_content.
 *                      Связь с категорией хранится в колонке category_id (ID категории из pages_categories).
 ...
 * Структура данных:    - title               -> Название страницы
 *                      - meta_title          -> SEO заголовок
 *                      - meta_description    -> SEO описание
 *                      - content             -> HTML содержимое страницы (из редактора)
 *                      - image               -> ID(шки) изображения из медиа-библиотеки (строка "1,2,3" или "5")
 *                      - url                 -> URL страницы
 *                      - category_id         -> ID категории из pages_categories
...
$titlemetah3        = 'Редактирование страницы';          // Название заголовка H2 для раздела
$titlemeta_h3       = 'Добавление страницы';              // Название заголовка H2 для раздела
$catalogTable       = 'pages_content';                    // Название таблицы страниц
$categoryTable      = 'pages_categories';                 // Название таблицы категорий
$categoryUrlPrefix  = 'page';                             // Префикс URL категории
...
if ($isEditMode && $itemId) {
    try {
        ...
        if ($item) {
            ...
            logEvent("Успешная загрузка страницы для редактирования ID=$itemId", LOG_INFO_ENABLED, 'info');
        } else {
            $errors[] = 'Страница не найдена';
            logEvent("Страница не найдена ID=$itemId", LOG_ERROR_ENABLED, 'error');
            header("Location: article_list.php");
            exit;
        }
    } catch (PDOException $e) {
        $errors[] = 'Ошибка загрузки данных';
        logEvent("Ошибка загрузки страницы ID=$itemId ({$catalogTable}): " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
    }
}
...
if (isset($_GET['action']) && $_GET['action'] === 'category_search') {
    ...
    try {
        ...
        // ВАЖНО: поиск категории идёт по pages_categories + users_id + status = 1
        if ($excludeId > 0) {
            $stmt = $pdo->prepare("
                SELECT id, name, url
                FROM {$categoryTable}
        ...
        if (empty($errors)) {
            try {
                if ($isEditMode && $itemId) {
                    ...
                    logEvent("Обновлена страница {$catalogTable} ID=$itemId users_id=$currentUserId", LOG_INFO_ENABLED, 'info');
                } else {
                    // INSERT (+ users_id)
                    $stmt = $pdo->prepare("
                    ...
                    logEvent("Создана страница {$catalogTable} ID=$newId users_id=$currentUserId", LOG_INFO_ENABLED, 'info');
                }

...
/**
 * Название файла:      add_extra.php
 * Назначение:          Добавление и редактирование записей в таблице с дополнительным контентом (pages_extra_content).
 *                      ФИЛЬТРАЦИЯ ПО users_id = $_SESSION['user_id'] при загрузке и users_id автоматически заполняется при создании.
 * Важно:               - page_id (родительский ID) берётся из GET параметра: ?page_id=1
 *                      - page_id сохраняется в отдельной колонке page_id.
 ...
 * Безопасность:        Имя таблицы нельзя принимать из GET/POST — задаётся в настройках.
...
$titlemeta       = 'Страницы';                             // Название заголовка H1 для раздела
...
$catalogTable    = 'pages_extra_content';                  // Название таблицы (подключение у записи)
$authorCheckTable = 'pages_content';                      // Привязка записи к родительской таблице
...
// GET параметры: page_id (обязательный), id (опционально для редактирования)
$pageIdFromGet = isset($_GET['page_id']) ? (int)$_GET['page_id'] : 0;

// Если существует GET параметр page_id и он содержит значение (ссылку)
if ($pageIdFromGet > 0) {
    try {
        ...
        $checkStmt->execute([$pageIdFromGet, $currentUserId]);
        $authorExists = $checkStmt->fetch(PDO::FETCH_ASSOC);
        
        // Если запись не найдена - редирект на list.php
        if (!$authorExists) {
            logEvent("Попытка доступа к несуществующему page_id ID: $pageIdFromGet для users_id: $currentUserId в таблице {$authorCheckTable} — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_INFO_ENABLED, 'info');
        ...
        logEvent("Ошибка проверки page_id в таблице {$authorCheckTable}: " . $e->getMessage() . " — pageId: $pageIdFromGet, users_id: $currentUserId — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_ERROR_ENABLED, 'error');
        header("Location: list.php");
        exit;
    }
}
...
if ($pageIdFromGet <= 0) {
    // page_id обязателен: именно его сохраняем в колонку page_id
    header("Location: list.php");
    exit;
}
...
if ($isEditMode && $itemId) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM {$catalogTable} WHERE id = ? AND page_id = ? AND users_id = ? LIMIT 1");
        $stmt->execute([$itemId, $pageIdFromGet, $currentUserId]);
        ...
        if ($item) {
            ...
            header("Location: extra_list.php?page_id=$pageIdFromGet");
            exit;
        }
    } catch (PDOException $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCsrfToken($csrfToken)) {
        ...
        if (empty($errors) && $pageIdFromGet > 0) {
            try {
                ...
                if (!$authorExists) {
                    $errors[] = 'Недействительный page_id параметр.';
                    logEvent("Попытка сохранения с несуществующим page_id ID: $pageIdFromGet для users_id: $currentUserId в таблице {$authorCheckTable}", LOG_INFO_ENABLED, 'info');
                }
            } catch (PDOException $e) {
                $errors[] = 'Ошибка проверки параметров.';
                logEvent("Ошибка проверки page_id перед сохранением в {$authorCheckTable}: " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
            }
        }
            ...
            try {
                if ($isEditMode && $itemId) {
                    ...
                    $stmt = $pdo->prepare("
                        ...
                        WHERE id = ? AND page_id = ? AND users_id = ?
                        LIMIT 1
                    ");
                    $stmt->execute([
                        $titlePost,
                    ...
                    logEvent("Обновлена запись {$catalogTable} ID=$itemId page_id={$pageIdFromGet} users_id={$currentUserId}", LOG_INFO_ENABLED, 'info');
                    header("Location: add_extra.php?page_id=$pageIdFromGet&id=$itemId");
                    exit;
                } else {
                    // INSERT с автоматическим заполнением users_id
                    $stmt = $pdo->prepare("
                        INSERT INTO {$catalogTable}
                            (page_id, title, content, image, sorting, status, users_id, created_at)
                        VALUES
                            (?, ?, ?, ?, ?, ?, ?, NOW())
                    ");
                    $stmt->execute([
                    ...
                    logEvent("Создана запись {$catalogTable} ID=$newId page_id={$pageIdFromGet} users_id={$currentUserId}", LOG_INFO_ENABLED, 'info');
                    header("Location: add_extra.php?page_id=$pageIdFromGet&id=$newId");
                    exit;
                }
            } catch (PDOException $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['title']) && $resultTitle !== null && $resultTitle['valid']) {
    ...
    // В остальных случаях используем значение из базы (это безопаснее чем не валидированные данные)
    $formTitle = $title;
}

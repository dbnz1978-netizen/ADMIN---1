 ...
 * Назначение:          Админ-страница со списком дополнительных элементов страниц (таблица pages_extra_content).
 ...
 *                      ФИЛЬТРАЦИЯ ПО users_id = $_SESSION['user_id'] И page_id из GET параметра.
 * Важно:               - Таблица pages_extra_content задаётся в настройках ($catalogTable) — меняется в одном месте.
 ...
 *                      - Поле page_id используется для фильтрации из GET параметра page_id.
...
// Проверка обязательного параметра page_id
...
if ($pageId > 0) {
    try {
        ...
        $checkStmt->execute([$pageId, $_SESSION['user_id']]);
        $authorExists = $checkStmt->fetch(PDO::FETCH_ASSOC);

        // Если запись не найдена - редирект на article_list.php
        if (!$authorExists) {
            logEvent("Попытка доступа к несуществующему page_id: $pageId для users_id: {$_SESSION['user_id']} в таблице {$authorCheckTable} — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_INFO_ENABLED, 'info');
        ...
        logEvent("Ошибка проверки page_id в таблице {$authorCheckTable}: " . $e->getMessage() . " — pageId: $pageId, users_id: {$_SESSION['user_id']} — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_ERROR_ENABLED, 'error');
        header("Location: article_list.php");
        exit;
    }
}
                ...
                try {
                    ...
                    switch ($action) {
                        case 'delete':
                            if ($isTrash) {
                                ...
                                foreach ($userIds as $userId) {
                                    $stmt = $pdo->prepare("DELETE FROM {$catalogTable} WHERE id = ? AND status = 0 AND users_id = ? AND page_id = ?");
                                    $stmt->execute([$userId, $currentUserId, $pageId]);
                                }
                                $successMessages[] = 'Записи успешно удалены';
                            } else {
                                // Перемещение в корзину
                                $stmt = $pdo->prepare("UPDATE {$catalogTable} SET status = 0 WHERE id IN ($placeholders) AND users_id = ? AND page_id = ?");
                                $stmt->execute(array_merge($userIds, [$currentUserId, $pageId]));
                        ...
                        case 'restore':
                            ...
                            $stmt = $pdo->prepare("UPDATE {$catalogTable} SET status = 1 WHERE id IN ($placeholders) AND users_id = ? AND page_id = ?");
                            $stmt->execute(array_merge($userIds, [$currentUserId, $pageId]));
                    ...
                    logEvent("Выполнено массовое действие '$action' администратором ID: $adminId над {$catalogTable} IDs: " . implode(',', $userIds) . " — users_id=$currentUserId page_id=$pageId — IP: $ip", LOG_INFO_ENABLED, 'info');
        ...
        <div class="content-card table-card">
            <div class="row align-items-center mb-4">
                ...
                <div class="col-lg-6 text-end">
                    <a href="add_extra.php?page_id=<?= $pageId ?>"
            ...
            <?php if (!$isTrash): ?>
                <?php
                    $trashCountStmt = $pdo->prepare("SELECT COUNT(*) FROM {$catalogTable} WHERE status = 0 AND users_id = ? AND page_id = ?");
                    $trashCountStmt->execute([$currentUserId, $pageId]);
                    $trashCount = (int)$trashCountStmt->fetchColumn();
                ?>
                <?php if ($trashCount > 0): ?>
                    <a href="?page_id=<?= $pageId ?>&trash=1" class="btn btn-outline-danger mb-3">
                        <i class="bi bi-trash"></i> Корзина
                        <span class="badge bg-danger"><?= escape((string)$trashCount) ?></span>
                    </a>
                <?php endif; ?>
            <?php else: ?>
                <a href="?page_id=<?= $pageId ?>" class="btn btn-outline-primary mb-3">
                        ...
                        <form method="GET" class="d-flex mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <?php if (!empty($search)): ?>
                                    ...
                                    <a href="?page_id=<?= $pageId ?><?= $isTrash ? '&trash=1' : '' ?>"
                                ...
                                <td colspan="6" class="text-center py-4">
                                    ...
                                    <p class="mt-2 auth-subtitle">
                                        ...
                                        <?php if ($currentUserId > 0): ?>
                                            <br><small class="text-muted">для пользователя ID: <?= $currentUserId ?> (page_id: <?= $pageId ?>)</small>
                                        <?php endif; ?>
                                    </p>
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($users as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="add_extra.php?id=<?= (int)$row['id'] ?>&page_id=<?= $pageId ?>" class="user-link text-decoration-none">
                                            <?= escape($row['title'] ?? '') ?>
                                        </a>
                                    ...
                                    <td>
                                        <div class="btn-group" style="gap: 4px;">
                                            <a href="add_extra.php?id=<?= (int)$row['id'] ?>&page_id=<?= $pageId ?>"
            ...
            echo renderPagination(
                ...
                array_filter([                                   // Массив GET-параметров для формирования ссылок пагинации. Можно менять
                    ...
                    'page_id' => $pageId,                        // Фильтр: поиск по родительскому автору
                ])
            ); 
            ?>

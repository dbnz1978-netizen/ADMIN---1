 ...
 * Назначение:          Скрипт удаления плагина Pages Plugin
...
function plugin_uninstall($pdo)
{
    try {
        ...
        $pdo->exec("DROP TABLE IF EXISTS `pages_extra_content`");
        $pdo->exec("DROP TABLE IF EXISTS `pages_content`");
        $pdo->exec("DROP TABLE IF EXISTS `pages_categories`");
        
        return [
            'success' => true,
            'message' => 'Плагин "Система управления страницами" успешно удален. Таблицы удалены.'
        ];

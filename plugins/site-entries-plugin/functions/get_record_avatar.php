 ...
 * @param string $tableName     Название таблицы (например: 'site_entries_categories')
...
function getRecordAvatar(PDO $pdo, int $recordId, int $userId, string $tableName, string $defaultImage = '../../../../admin/img/galereya.svg'): string {
    ...
    $query = null;
    switch ($tableName) {
        case 'site_entries_categories':
            $query = "SELECT image FROM site_entries_categories WHERE id = ? AND users_id = ?";
            break;
        case 'site_entries_articles':
            $query = "SELECT image FROM site_entries_articles WHERE id = ? AND users_id = ?";
            break;
        case 'site_entries_extra_content':
            $query = "SELECT image FROM site_entries_extra_content WHERE id = ? AND users_id = ?";
            break;
        default:

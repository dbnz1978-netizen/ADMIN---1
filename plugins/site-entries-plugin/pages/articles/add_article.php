 ...
 * Назначение:          Добавление и редактирование записей в админ-панели.
 *                      Работает с таблицей site_entries_articles.
 *                      Связь с категорией хранится в колонке category_id (ID категории из site_entries_categories).
 ...
 * Структура данных:    - title               -> Название записи сайта
 *                      - meta_title          -> SEO заголовок
 *                      - meta_description    -> SEO описание
 *                      - content             -> HTML содержимое записи (из редактора)
 *                      - image               -> ID(шки) изображения из медиа-библиотеки (строка "1,2,3" или "5")
 *                      - url                 -> URL записи
 *                      - category_id         -> ID категории из site_entries_categories
...
$titlemetah3        = 'Редактирование записи сайта';           // Название заголовка H2 для раздела
$titlemeta_h3       = 'Добавление записи сайта';               // Название заголовка H2 для раздела
$catalogTable       = 'site_entries_articles';                    // Название таблицы записей
$categoryTable      = 'site_entries_categories';                  // Название таблицы категорий
...
if (isset($_GET['action']) && $_GET['action'] === 'category_search') {
    ...
    try {
        ...
        // ВАЖНО: поиск категории идёт по site_entries_categories + users_id + status = 1
        if ($excludeId > 0) {
            $stmt = $pdo->prepare("
                SELECT id, name, url
                FROM {$categoryTable}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCsrfToken($csrfToken)) {
        ...
        // Название записи сайта
        $title = trim($_POST['title'] ?? '');
        $result = validateTextareaField($title, 1, 200, 'Название записи сайта');
        if ($result['valid']) {
            $title = $result['value'];
            logEvent("Успешная валидация поля 'Название записи сайта'", LOG_INFO_ENABLED, 'info');
        } else {
            $errors[] = $result['error'];
            $title = false;
            logEvent("Ошибка валидации поля 'Название записи сайта': " . $result['error'], LOG_ERROR_ENABLED, 'error');
        }
        ...
        if (empty($errors)) {
            try {
                ...
                logEvent("Ошибка сохранения записи {$catalogTable}: " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
            }
        }
    }
}
        ...
        <main class="main-content">
            ...
            <form method="post">
                <div class="form-section">
                        ...
                        <div class="col-lg-6 text-end">
                            <?php if ($isEditMode && $itemId && $categoryFullPath): ?>
                                <a href="/<?= escape($categoryUrlPrefix) ?>/<?= escape($categoryFullPath) ?>/<?= escape($defaultUrl) ?>" target="_blank"
                                    ...
                                    title="Открыть страницу записи в новом окне">
                    ...
                    <div class="row mb-3">
                        <div class="col-12">
                            <label class="form-label">Название записи сайта <span class="text-danger">*</span></label>
                ...
                <div class="mb-5">
                    ...
                    <div class="form-text">Полное содержимое записи.</div>

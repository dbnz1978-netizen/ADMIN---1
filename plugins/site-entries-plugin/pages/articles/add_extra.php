 ...
 * Назначение:          Добавление и редактирование записей в таблице с дополнительным контентом (site_entries_extra_content).
 *                      ФИЛЬТРАЦИЯ ПО users_id = $_SESSION['user_id'] при загрузке и users_id автоматически заполняется при создании.
 * Важно:               - entry_id (родительский ID) берётся из GET параметра: ?entry_id=1
 *                      - entry_id сохраняется в отдельной колонке entry_id.
...
// GET параметры: entry_id (обязательный), id (опционально для редактирования)
$newsIdFromGet = isset($_GET['entry_id']) ? (int)$_GET['entry_id'] : 0;
...
if ($newsIdFromGet > 0) {
    try {
        ...
        if (!$authorExists) {
            logEvent("Попытка доступа к несуществующему entry_id ID: $newsIdFromGet для users_id: $currentUserId в таблице {$authorCheckTable} — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_INFO_ENABLED, 'info');
        ...
        logEvent("Ошибка проверки entry_id в таблице {$authorCheckTable}: " . $e->getMessage() . " — newsId: $newsIdFromGet, users_id: $currentUserId — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_ERROR_ENABLED, 'error');
        header("Location: list.php");
        exit;
    }
}
...
if ($newsIdFromGet <= 0) {
    // entry_id обязателен: именно его сохраняем в колонку entry_id
    header("Location: list.php");
    exit;
}
...
if ($isEditMode && $itemId) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM {$catalogTable} WHERE id = ? AND entry_id = ? AND users_id = ? LIMIT 1");
        ...
        if ($item) {
            ...
            header("Location: extra_list.php?entry_id=$newsIdFromGet");
            exit;
        }
    } catch (PDOException $e) {
    ...
    if (!validateCsrfToken($csrfToken)) {
        ...
        if (empty($errors) && $newsIdFromGet > 0) {
            try {
                ...
                if (!$authorExists) {
                    $errors[] = 'Недействительный entry_id параметр.';
                    logEvent("Попытка сохранения с несуществующим entry_id ID: $newsIdFromGet для users_id: $currentUserId в таблице {$authorCheckTable}", LOG_INFO_ENABLED, 'info');
                }
            } catch (PDOException $e) {
                $errors[] = 'Ошибка проверки параметров.';
                logEvent("Ошибка проверки entry_id перед сохранением в {$authorCheckTable}: " . $e->getMessage(), LOG_ERROR_ENABLED, 'error');
            }
        }
            ...
            try {
                if ($isEditMode && $itemId) {
                    ...
                    $stmt = $pdo->prepare("
                        ...
                        WHERE id = ? AND entry_id = ? AND users_id = ?
                        LIMIT 1
                    ");
                    $stmt->execute([
                        $titlePost,
                    ...
                    logEvent("Обновлена запись {$catalogTable} ID=$itemId entry_id={$newsIdFromGet} users_id={$currentUserId}", LOG_INFO_ENABLED, 'info');
                    header("Location: add_extra.php?entry_id=$newsIdFromGet&id=$itemId");
                    exit;
                } else {
                    // INSERT с автоматическим заполнением users_id
                    $stmt = $pdo->prepare("
                        INSERT INTO {$catalogTable}
                            (entry_id, title, content, image, sorting, status, users_id, created_at)
                        VALUES
                            (?, ?, ?, ?, ?, ?, ?, NOW())
                    ");
                    $stmt->execute([
                    ...
                    logEvent("Создана запись {$catalogTable} ID=$newId entry_id={$newsIdFromGet} users_id={$currentUserId}", LOG_INFO_ENABLED, 'info');
                    header("Location: add_extra.php?entry_id=$newsIdFromGet&id=$newId");
                    exit;
                }
            } catch (PDOException $e) {

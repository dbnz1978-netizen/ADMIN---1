 ...
 * Назначение:          Админ-страница со списком записей.
...
$titlemetah3       = 'Управление записями сайта';    // Название заголовка H2 для раздела
...
// Поиск по названию категории - валидация и очистка
$search_catalog = '';
if (isset($_GET['search_catalog'])) {
    $searchCatalogResult = validateTextareaField($_GET['search_catalog'], 0, 100, 'Поиск по категории');
...
// [НОВОЕ ПОЛЕ] - Добавьте здесь новые поля, которые хотите получить из таблицы site_entries_articles
try {
    $query = "
        SELECT
            id,
    ...
    if ($search_catalog !== '') {
        ...
        $query .= " AND EXISTS (
                        ...
                        WHERE c.id = {$catalogTable}.category_id
                           AND c.status = 1
                           AND c.users_id = :users_id_search
                           AND c.name LIKE :search_catalog_search
                    )";
    ...
    <main class="main-content">
        ...
        <div class="content-card table-card">
            ...
            <?php if (!$isTrash): ?>
                ...
                <?php endif; ?>
            <?php else: ?>
                <a href="article_list.php" class="btn btn-outline-primary mb-3">
                        ...
                        <form method="GET" class="d-flex mb-3">
                            ...
                            <div class="input-group" style="gap: 2px;">
                                ...
                                <?php if ($search !== '' || $search_catalog !== ''): ?>
                                    <a href="<?= escape($isTrash ? 'article_list.php?trash=1' : 'article_list.php') ?>"
                                ...
                                <tr>
                                    ...
                                    <?php
                                        ...
                                        $rowImage = getRecordAvatar(
                                            ...
                                            $catalogTable              // Название таблицы (например: 'site_entries_articles')
                                        );
                                    ?>
                                    ...
                                    <td>
                                        <a href="add_article.php?id=<?= (int)$row['id'] ?>" class="user-link text-decoration-none">
                                            <?= escape($row['title'] ?? '') ?>
                                        </a>
                                        ...
                                        /**
                                         * 
                                        <!-- [НОВОЕ ПОЛЕ] - Вывод новых полей из таблицы -->
                                        <?php if (!empty($row['field_name'])): ?>
                                            <div class="text-muted small">Поле: <?= escape($row['field_name']) ?></div>
                                        <?php endif; ?>
                                         */
                                        ?>
                                    ...
                                    <td class="text-center">
                                        <?php if (!empty($row['url'])): ?>
                                            <a href="/<?= escape($categoryUrlPrefix) ?>/<?= escape($row['url']) ?>"
                                               target="_blank"
                                    ...
                                    <td>
                                        <div class="btn-group" style="gap: 4px;">
                                            <a href="add_article.php?id=<?= (int)$row['id'] ?>"

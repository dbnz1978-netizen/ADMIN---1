...
/**
 * Название файла:      extra_list.php
 * Назначение:          Админ-страница со списком дополнительных элементов записей (таблица site_entries_extra_content).
 ...
 *                      ФИЛЬТРАЦИЯ ПО users_id = $_SESSION['user_id'] И entry_id из GET параметра.
 * Важно:               - Таблица site_entries_extra_content задаётся в настройках ($catalogTable) — меняется в одном месте.
 ...
 *                      - Поле entry_id используется для фильтрации из GET параметра entry_id.
...
$titlemeta        = 'Записи сайта';                      // Название заголовка H1 для раздела
$titlemetah3      = 'Дополнительный контент';       // Название заголовка H2 для раздела
$catalogTable     = 'site_entries_extra_content';           // Название таблицы (подключение у записи)
$authorCheckTable = 'site_entries_articles';                // Привязка записи к родительской таблице
...
// Проверка обязательного параметра entry_id
$newsId = isset($_GET['entry_id']) ? (int)$_GET['entry_id'] : 0;
// Валидация newsId
if ($newsId < 0) {
    $newsId = 0;
}

// Если существует GET параметр entry_id и он содержит значение (ссылку)
if ($newsId > 0) {
    try {
        ...
        if (!$authorExists) {
            logEvent("Попытка доступа к несуществующему entry_id: $newsId для users_id: {$_SESSION['user_id']} в таблице {$authorCheckTable} — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_INFO_ENABLED, 'info');
        ...
        logEvent("Ошибка проверки entry_id в таблице {$authorCheckTable}: " . $e->getMessage() . " — newsId: $newsId, users_id: {$_SESSION['user_id']} — IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'), LOG_ERROR_ENABLED, 'error');
        header("Location: article_list.php");
        exit;
    }
}
                ...
                try {
                    ...
                    switch ($action) {
                        case 'delete':
                            if ($isTrash) {
                                ...
                                foreach ($userIds as $userId) {
                                    $stmt = $pdo->prepare("DELETE FROM {$catalogTable} WHERE id = ? AND status = 0 AND users_id = ? AND entry_id = ?");
                                    $stmt->execute([$userId, $currentUserId, $newsId]);
                                }
                                $successMessages[] = 'Записи успешно удалены';
                            } else {
                                // Перемещение в корзину
                                $stmt = $pdo->prepare("UPDATE {$catalogTable} SET status = 0 WHERE id IN ($placeholders) AND users_id = ? AND entry_id = ?");
                        ...
                        case 'restore':
                            ...
                            $stmt = $pdo->prepare("UPDATE {$catalogTable} SET status = 1 WHERE id IN ($placeholders) AND users_id = ? AND entry_id = ?");
                            $stmt->execute(array_merge($userIds, [$currentUserId, $newsId]));
                    ...
                    logEvent("Выполнено массовое действие '$action' администратором ID: $adminId над {$catalogTable} IDs: " . implode(',', $userIds) . " — users_id=$currentUserId entry_id=$newsId — IP: $ip", LOG_INFO_ENABLED, 'info');
...
<div class="container-fluid">
    ...
    <main class="main-content">
        ...
        <?php 
            if (isset($_GET['entry_id']) && !empty($_GET['entry_id']) && is_numeric($_GET['entry_id'])) { 
        ...
        <div class="content-card table-card">
            <div class="row align-items-center mb-4">
                ...
                <div class="col-lg-6 text-end">
                    <a href="add_extra.php?entry_id=<?= $newsId ?>"
            ...
            <?php if (!$isTrash): ?>
                <?php
                    $trashCountStmt = $pdo->prepare("SELECT COUNT(*) FROM {$catalogTable} WHERE status = 0 AND users_id = ? AND entry_id = ?");
                    $trashCountStmt->execute([$currentUserId, $newsId]);
                    $trashCount = (int)$trashCountStmt->fetchColumn();
                ?>
                <?php if ($trashCount > 0): ?>
                    <a href="?entry_id=<?= $newsId ?>&trash=1" class="btn btn-outline-danger mb-3">
                        <i class="bi bi-trash"></i> Корзина
                        <span class="badge bg-danger"><?= escape((string)$trashCount) ?></span>
                    </a>
                <?php endif; ?>
            <?php else: ?>
                <a href="?entry_id=<?= $newsId ?>" class="btn btn-outline-primary mb-3">
                ...
                <div class="row align-items-center">
                    <div class="col-md-6">
                        ...
                        <form method="GET" class="d-flex mb-3">
                            <input type="hidden" name="entry_id" value="<?= $newsId ?>">
                            <?php if ($isTrash): ?>
                            ...
                            <div class="input-group">
                                ...
                                <?php if (!empty($search)): ?>
                                    ...
                                    <a href="?entry_id=<?= $newsId ?><?= $isTrash ? '&trash=1' : '' ?>"
                                ...
                                <td colspan="6" class="text-center py-4">
                                    ...
                                    <p class="mt-2 auth-subtitle">
                                        ...
                                        <?php if ($currentUserId > 0): ?>
                                            <br><small class="text-muted">для пользователя ID: <?= $currentUserId ?> (entry_id: <?= $newsId ?>)</small>
                                        <?php endif; ?>
                                    </p>
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($users as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="add_extra.php?id=<?= (int)$row['id'] ?>&entry_id=<?= $newsId ?>" class="user-link text-decoration-none">
                                            <?= escape($row['title'] ?? '') ?>
                                        </a>
                                    ...
                                    <td>
                                        <div class="btn-group" style="gap: 4px;">
                                            <a href="add_extra.php?id=<?= (int)$row['id'] ?>&entry_id=<?= $newsId ?>"
            ...
            echo renderPagination(
                ...
                array_filter([                                   // Массив GET-параметров для формирования ссылок пагинации. Можно менять
                    ...
                    'entry_id' => $newsId,                        // Фильтр: поиск по родительскому автору
                ])
            ); 
            ?>
